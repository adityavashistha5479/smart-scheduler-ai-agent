...
*   **Stateful Memory:** Persists user preferences (like standard meeting length and preferred times) via a SQLite/PostgreSQL database to enable "usual sync-up" scheduling without redundant questions.
...
4.  **Database for Memory:** A SQLAlchemy database stores user preferences (e.g., standard meeting duration), injected dynamically into the LLM system prompt on connection. This allows the AI to handle ambiguous requests like "schedule our usual sync-up".
